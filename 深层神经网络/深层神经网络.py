...
def forward_propagation(A, W, b,activation_func):   #值负责核心计算功能的向前传播函数
    Z = np.dot(W, A) + b
    A = activation_func(Z)
    return A, Z


def forward_propagation_complete(X,Y, parameters,chosen_func,Z_cache,A_cache): #管理所有向前传播细节的函数
